...
_config = Configuration(
	...
	Section('web', 'Settings related to serving content to the web.',
		StringConfigItem("serverURL", default="http://localhost:8080", description="URL fragment used to qualify relative URLs where necessary.  Note that this must contain the port the server is accessible under from the outside if that is not 80."),
		StringConfigItem("bindAddress", default="127.0.0.1", description="Interface to bind to"),
		IntConfigItem("serverPort", default="8080",description="Port to bind the server to"),
		StringConfigItem("user", default="gavo", description="Run server as this user."),
		EnsureTrailingSlashesItem("nevowRoot", default="/", description="Path fragment to the server's root for operation off the server's root; this must end with a slash (and, frankly, if you must use this feature, you'll probably encounter some bugs. we want to fix those, though.)"),
		StringConfigItem("realm", default="X-Unconfigured", description="Authentication realm to be used (currently, only one, server-wide, is supported)"),
		WebRelativeConfigItem("templateDir", default="templates", description="webDir-relative location of global nevow templates"),
		StringConfigItem("adminpasswd", default="", description="Password for online administration, leave empty to disable"),
		StringConfigItem("sitename", "Unnamed data center","A short name for your site"),
		RelativeURL("voplotCodeBase", "None","Deprecated and ignored."),
		RelativeURL("voplotUserman","Deprecated and ignored","URL to the documentation of VOPlot"),
